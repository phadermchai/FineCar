...
<nav class="navbar navbar-light navbar-expand-md fixed-top" style="background-color:rgba(0,0,0,0.49);color:rgba(0,0,0,0);">
    ...
    <section id="headtext">
        <div>
            <h2 id="htext">การจองของคุณ <?php echo $_SESSION["Username"]; ?></h2>
            <div class="table-responsive">
                <table class="table">
                    ...
                    <?php
                        $sql = " SELECT * FROM bookinfo JOIN carinfo ON bookinfo.Username = '$Username' AND bookinfo.CarID = carinfo.CarID ";
    ...
    <section id="headtext2" style="background-color:#eeedfe;">
        <div>
            <h2 id="htext">รถของคุณ</h2>
            <div class="table-responsive">
                <table class="table">
                    <thead>
                        <tr>
                            <th>รหัสรถของคุณ</th>
                            <th>รถยี่ห้อ</th>
                            <th>ดูออเดอร์การจองรถของคุณ</th>
                            <th>ดูรายละเอียดรถของคุณ</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php
                     $sql = " SELECT * FROM carinfo WHERE Username = '$Username' ";
                     $result = mysqli_query($conn, $sql) or die("Bad Query: $sql");
                     if($result){
                        while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                        echo '<tr>
                            <td>'.$row['CarID'].'</td>
                            <td>'.$row['CarName'].'</td>
                            <td><a href="OrderUserCar.php">รายละเอียด</a></td>
                            <td><a href="CarDetail.php?ID='.$row['CarID'].'">รายละเอียด</a></td>
                        </tr>';
                         }
                    }
                    ?>
    ...
    <section id="headtext2">
        <div>
            <h2 id="htext">คนขับ</h2>
            <div class="table-responsive">
                <table class="table">
                    <thead>
                        <tr>
                            <th>รหัสคนขับของคุณ</th>
                            <th>ชื่อ</th>
                            <th>เขตการทำงาน</th>
                            <th>เบอร์ติดต่อ</th>
                            <th>ดูข้อมูลคนขับของคุณ</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php
                    $sql = " SELECT * FROM driverinfo WHERE Username = '$Username' ";
                    $result = mysqli_query($conn, $sql) or die("Bad Query: $sql");
                    if($result){
                        while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                       echo' <tr>
                            <td>'.$row['DriverID'].'</td>
                            <td>'.$row['D_Firstname'].' '.$row['D_Lastname'].'</td>
                            <td>'.$row['D_Loca'].', Thailand</td>
                            <td>'.$row['D_Tel'].'</td>
                            <td><a href="'.$row['Pic'].'">รายละเอียด</a></td>
                        </tr>' ;
                        }
                    }
                    ?>
